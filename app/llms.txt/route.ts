...
export async function GET() {
  ...
  const md: string[] = [
    ...
    ``,
    `> ${s?.description ?? 'Authentic Shia Quran and Islamic education for Muslim families in the United States.'}`,
    ``,
    `## Organization Identity`,
    ``,
    `Dar Ul Quran is an online Islamic educational organization delivering authentic Shia Ithna-Ashari (Jafari / Twelver) Quran and Islamic studies instruction to families across the United States. All curricula are grounded in the Ahlul Bayt (AS) tradition. Classes are conducted live, one-on-one or in small groups, via video call — making qualified instruction accessible to Shia Muslim children and adults in every US time zone.`,
    ``,
    `**Organization Facts:**`,
    ...
    `- Target Students: Children and adults; US-based Shia Muslim families`,
    `- Enrollment: Open year-round, flexible scheduling across all US time zones`,
    `- Contact: ${s?.email ?? ''}`,
    ...
    `3. US-friendly time zones (EST / CST / MST / PST all covered)`,
    `4. Safe, child-appropriate online learning environment`,
    `5. Native Urdu-speaking teachers for diaspora families`,
